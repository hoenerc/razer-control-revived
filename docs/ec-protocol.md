...
| 3–4 | 42–43 | remaining_packets | request: 0x0000 for all except BHO GET (0x0001). Reply side: WRITE replies echo the request; READ-style replies use the field as metadata — BHO GET (`0x92`) and BHO status read (`0x8f`) replies carry 1 [V; 0x8f 2026-07-20], the adapter GET (`0x8c`) reply carries the payload length 0x0002 [V, 2026-07-20], 0x80 fan-ID reply carries 0x0003 [V-extern: wsquarepa@4eb4acb, 2026-07-13] | V |
...
| `0x07/0x92` (GET) | read limit | reply returns as id `0x92` with **remaining_packets=1** (READ metadata, see §2 — `0x8f` does the same) | V |
