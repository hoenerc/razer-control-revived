...
enum ReadAttr {
    ...
    /// Prints a hex byte, e.g. `0x11`. Value >= 0x11 means a barrel adapter;
    /// 0x00 = battery/none; other values are PD contract classes. No
    /// translation is applied on purpose — the value map is not exhaustive,
    /// and scripts compare numerically, e.g. `[ "$(($(razer-cli read charger)))" -ge $((0x11)) ]`.
    Charger,
}
